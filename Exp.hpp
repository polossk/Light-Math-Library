...
template <typename TrueType, typename DType, int exp_type> struct Exp {
  inline const TrueType &self() const {
    return *static_cast<const TrueType *>(this);
  }
  inline TrueType *ptrself() { return static_cast<TrueType *>(this); }
};

template <typename Saver, typename RValue, typename DType> struct ExpEngine;
...
template <typename DType> inline ScalarExp<DType> scalar(DType _) {
...
struct TypecastExp
    : public Exp<TypecastExp<DstDType, SrcDType, EType, exp_type>, DstDType,
                 exp_type> {
  const EType &expr;
...
typecast(const Exp<EType, SrcDType, exp_type> &exp) {
  return TypecastExp<DstDType, SrcDType, EType, (exp_type | type::kMapper)>(
      exp.self());
}
...
public:
  ...
  inline Container &operator+=(DType s) {
    ExpEngine<sv::plusto, Container, DType>::Eval(this->ptrself(),
                                                  scalar<DType>(s));
    return *(this->ptrself());
  }

  inline Container &operator-=(DType s) {
    ExpEngine<sv::minusto, Container, DType>::Eval(this->ptrself(),
                                                   scalar<DType>(s));
    return *(this->ptrself());
  }

  inline Container &operator*=(DType s) {
    ExpEngine<sv::multo, Container, DType>::Eval(this->ptrself(),
                                                 scalar<DType>(s));
    return *(this->ptrself());
  }

  inline Container &operator/=(DType s) {
    ExpEngine<sv::divto, Container, DType>::Eval(this->ptrself(),
                                                 scalar<DType>(s));
    return *(this->ptrself());
  }

  inline Container &__assign(DType s) {
    ExpEngine<sv::saveto, Container, DType>::Eval(this->ptrself(),
                                                  scalar<DType>(s));
    return *(this->ptrself());
  }
...
// comment todo
template <typename Tlhs, typename Trhs, bool ltrans, bool rtrans,
          typename DType>
struct DotExp : public Exp<DotExp<Tlhs, Trhs, ltrans, rtrans, DType>, DType,
                           type::kComplex> {
  const Tlhs &lhs_;
  const Trhs &rhs_;
  DType scale_;
...
dot(const RValueExp<Tlhs, DType> &lhs, const RValueExp<Trhs, DType> &rhs) {
  return DotExp<Tlhs, Trhs, false, false, DType>(lhs.self(), rhs.self(),
                                                 DType(1.0f));
}
...
dot(const RValueExp<Tlhs, DType> &lhs, const RValueExp<Trhs, DType> &rhs) {
  return DotExp<Tlhs, Trhs, false, true, DType>(lhs.self(), rhs.expr,
                                                DType(1.0f));
}
...
dot(const RValueExp<Tlhs, DType> &lhs, const RValueExp<Trhs, DType> &rhs) {
  return DotExp<Tlhs, Trhs, true, false, DType>(lhs.expr, rhs.self(),
                                                DType(1.0f));
}
...
template <bool transpose_left, bool transpose_right, typename Tlhs,
          typename Trhs, typename DType>
inline DotExp<Tlhs, Trhs, transpose_left, transpose_right, DType>
batch_dot(const RValueExp<Tlhs, DType> &lhs,
          const RValueExp<Trhs, DType> &rhs) {
  return DotExp<Tlhs, Trhs, transpose_left, transpose_right, DType>(
...
MakeExp(const Exp<TA, DType, eta> &_1, const Exp<TB, DType, etb> &_2,
        const Exp<TC, DType, etc> &_3) {
  return TernaryMapExp<OP, TA, TB, TC, DType,
                       (eta | etb | etc | type::kMapper)>(_1.self(), _2.self(),
                                                          _3.self());
}
...
template <typename OP, typename TA, typename TB, typename TC, typename DType,
          int eta, int etb, int etc>
inline TernaryMapExp<OP, TA, TB, TC, DType, (eta | etb | etc | type::kMapper)>
F(const Exp<TA, DType, eta> &_1, const Exp<TB, DType, etb> &_2,
  const Exp<TC, DType, etc> &_3) {
...
struct BinaryMapExp
    : public Exp<BinaryMapExp<OP, Tlhs, Trhs, DType, etype>, DType, etype> {
  const Tlhs &lhs_;
  const Trhs &rhs_;
  explicit BinaryMapExp(const Tlhs &lhs, const Trhs &rhs)
      : lhs(lhs_), rhs(rhs_) {}
};

template <typename OP, typename Tlhs, typename Trhs, typename DType, int etlhs,
          int etrhs>
inline BinaryMapExp<OP, Tlhs, Trhs, DType, (etlhs | etrhs | type::kMapper)>
MakeExp(const Exp<Tlhs, DType, etlhs> &lhs,
        const Exp<Trhs, DType, etrhs> &rhs) {
  return BinaryMapExp<OP, Tlhs, Trhs, DType, (etlhs | etrhs | type::kMapper)>(
      lhs.self(), rhs.self());
}
...
inline BinaryMapExp<op::plus, Tlhs, Trhs, DType,
                    (etlhs | etrhs | type::kMapper)>
operator+(const Exp<Tlhs, DType, etlhs> &lhs,
          const Exp<Trhs, DType, etrhs> &rhs) {
  return MakeExp<op::plus>(lhs, rhs);
}

template <typename Tlhs, typename Trhs, typename DType, int etlhs, int etrhs>
inline BinaryMapExp<op::minus, Tlhs, Trhs, DType,
                    (etlhs | etrhs | type::kMapper)>
operator-(const Exp<Tlhs, DType, etlhs> &lhs,
          const Exp<Trhs, DType, etrhs> &rhs) {
